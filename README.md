...
- 🎯 Entry points to execute your program using `python -m <iot_backend>` or `$ iot_backend` with basic CLI argument parsing.
...
---
# iot_backend Flask Application

Awesome iot_backend created by fatihfurkanaydemir
...
```bash
git clone https://github.com/fatihfurkanaydemir/iot-backend iot_backend
cd iot_backend
make install
```
...
```bash
iot_backend create-db   # run once
iot_backend populate-db  # run once (optional)
iot_backend add-user -u admin -p 1234  # ads a user
iot_backend run
```
