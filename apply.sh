...
original_author="fatihfurkanaydemir"
original_name="iot_backend"
original_urlname="iot-backend"
original_description="Awesome iot_backend created by fatihfurkanaydemir" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/iot_backend "${name}"
...
then
    ...
    echo "then run 'iot_backend' or 'python -m iot_backend'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'iot_backend' or 'python -m iot_backend' or 'poetry run iot_backend'"
fi
